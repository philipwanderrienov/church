...
const emptyBoard: Omit<BoardMember, "id"> = {
  name: "",
  position: "",
  commission: "",
  termStart: "",
...
const emptyCommission: Omit<Commission, "id"> = {
  name: "",
  description: "",
  head: "",
  memberCount: 0,
...
export default function Organization() {
  ...
  const saveBoard = () => {
    ...
    if (editingBoard)
      update<BoardMember>("board_members", editingBoard.id, boardForm);
  ...
  const saveComm = () => {
    ...
    if (editingComm)
      update<Commission>("commissions", editingComm.id, commForm);
    ...
    <PageLayout>
      <div className="container mx-auto px-4">
        ...
        <p className="text-muted-foreground mb-8">
          Manage board members, terms of office, and commissions
        </p>
                  ...
                  {boards.map((b) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <span
                          className={`text-xs font-medium px-2 py-1 rounded-full ${b.status === "Active" ? "bg-green-100 text-green-700" : "bg-destructive/10 text-destructive"}`}
                        >
                          {b.status}
                        </span>
                      </td>
      ...
      <CrudDialog
        ...
        title={editingBoard ? "Edit Board Member" : "Add Board Member"}
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setBoardForm({ ...boardForm, position: e.target.value })
              }
            />
          </div>
          <div>
            ...
            <Input
              value={boardForm.commission}
              onChange={(e) =>
                setBoardForm({ ...boardForm, commission: e.target.value })
              }
            />
          </div>
          <div>
            ...
            <Input
              type="date"
              value={boardForm.termStart}
              onChange={(e) =>
                setBoardForm({ ...boardForm, termStart: e.target.value })
              }
            />
          </div>
          <div>
            ...
            <Input
              type="date"
              value={boardForm.termEnd}
              onChange={(e) =>
                setBoardForm({ ...boardForm, termEnd: e.target.value })
              }
            />
          </div>
          <div>
        ...
        <div className="flex justify-end gap-3 mt-6">
          <Button variant="outline" onClick={() => setBoardDialog(false)}>
            Cancel
          </Button>
          <Button
            onClick={saveBoard}
      ...
      <CrudDialog
        ...
        title={editingComm ? "Edit Commission" : "Add Commission"}
      >
        <div className="grid grid-cols-1 gap-4 mt-4">
          ...
          <div>
            ...
            <Input
              value={commForm.head}
              onChange={(e) =>
                setCommForm({ ...commForm, head: e.target.value })
              }
            />
          </div>
          <div>
        ...
        <div className="flex justify-end gap-3 mt-6">
          <Button variant="outline" onClick={() => setCommDialog(false)}>
            Cancel
          </Button>
          <Button
            onClick={saveComm}
