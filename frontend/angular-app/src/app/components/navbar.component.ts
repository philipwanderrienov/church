    ...
    <header class="navbar">
      <div class="navbar__content container">
        <div class="navbar__text">
          ...
          <nav *ngIf="breadcrumbs?.length" aria-label="Breadcrumb" class="navbar__breadcrumbs">
            <ng-container *ngFor="let item of breadcrumbs; let last = last">
              <a *ngIf="item.href && !last" [routerLink]="item.href">{{ item.label }}</a>
              <span *ngIf="!item.href || last" [class.is-last]="last">{{ item.label }}</span>
        ...
        <div class="navbar__actions" *ngIf="currentUser">
          <div class="navbar__user">
            ...
            <div class="navbar__user-role">{{ roleText || currentUser.role }}</div>
          </div>
          <span class="navbar__badge">{{ roleText || currentUser.role }}</span>
          <button type="button" class="navbar__logout" (click)="logout()">Keluar</button>
        </div>
      </div>
    </header>
  `,
